...
public interface Spliterator<T> {
    ...
    // SIZED 特性表示该 Spliterator 所表示的数据源的大小已知。
    ...
    // SUBSIZED 特性表示每次拆分后的子 Spliterator 也具有 SIZED 特性
    int SUBSIZED = 0x00004000;

    /**
     * 若有下一个元素，执行action操作，返回 {@code true}，否则返回{@code false}
     * @throws NullPointerException 如果指定的操作为 null
     */
    boolean tryAdvance(Consumer<? super T> action);

    /**
     * 为接下来的所有元素执行action操作，直到全部执行完毕或者引发异常
     * @throws NullPointerException 如果指定的操作为 null
     */
    ...
    /**
     * 将分解器尝试进行分解成多个分解器，比较小的不能分解的分解器返回 {@code null}
     * @return 一个覆盖部分元素的 {@code Spliterator}，如果这个 spliterator 无法分割，则返回 {@code null}
     */
    Spliterator<T> trySplit();

    /**
     * 返回当前未遍历的元素的数量，如果元素数量是无限的、未知的，或者计算成本过高，返回 {@code Long.MAX_VALUE}
     * @return 估计的大小，如果元素数量是无限的、未知的，或者计算成本过高，则返回 {@code Long.MAX_VALUE}。
     */
    long estimateSize();

    /**
     * 如果特征包含为 SIZED 返回 {@code estimateSize()}， 否则返回 {@code -1}
     */
