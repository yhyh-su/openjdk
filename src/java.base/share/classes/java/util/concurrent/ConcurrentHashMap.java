...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * 表的初始化和扩容控制。当值为负时，表示表正在被初始化或扩容：
     * -1 表示正在初始化，
     * 其他负值表示正在进行扩容的线程数（-（1 + 活动扩容线程的数量））。
     * 否则，当表为 null 时，保存创建时要使用的初始表大小，或者 0 表示使用默认大小。
     * 初始化后，保存下次扩容所需的元素数量。
     */
    ...
    /**
     * 初始化哈希表，使用 sizeCtl 中记录的大小。
     * 如果表为空或未初始化，方法会确保哈希表的初始化过程安全且高效。
     */
    private final Node<K, V>[] initTable() {
        Node<K, V>[] tab;  // 表的引用
        int sc;  // 用于存储 sizeCtl 的值，控制表的大小

        // 不断循环，直到初始化完成
        while ((tab = table) == null || tab.length == 0) {
            // 如果 sizeCtl 小于 0，表示初始化失败，需要进行线程自旋等待
            if ((sc = sizeCtl) < 0)
                Thread.yield();  // 放弃当前线程的时间片，稍后重试

                // 如果当前线程赢得了初始化竞争，通过 CAS 更新 sizeCtl
            else if (U.compareAndSetInt(this, SIZECTL, sc, -1)) {
                try {
                    // 检查 table 是否仍然为空或未初始化
                    if ((tab = table) == null || tab.length == 0) {
                        // 如果 sc 大于 0，使用 sc 作为初始化大小，否则使用默认的容量
                        int n = (sc > 0) ? sc : DEFAULT_CAPACITY;

                        // 强制类型转换为合适的 Node 数组类型，Node 是哈希表中的元素类型
                        @SuppressWarnings("unchecked")
                        Node<K, V>[] nt = (Node<K, V>[]) new Node<?, ?>[n];

                        // 设置 table 引用为新创建的哈希表
                        table = tab = nt;
                        // 设置新的 sc 值为 n 的 75%，即填充阈值
                        sc = n - (n >>> 2);
                    }
                } finally {
                    // 最后更新 sizeCtl，避免死锁或其他线程不能继续执行
                    sizeCtl = sc;
                }
                break;  // 初始化成功，退出循环
            }
